...
def search(request):
    if request.method == "POST":                                                       # The "post"is from the html layout template #the id of the textfield we created in the html layout template  #if we use get we get the exact match, if we want just to search  "like" we use the fitler word #The result will be a speaker object that has contains the data to the search result of what the user typed
        keyword = request.POST["q"]                                                    
        results = Speaker.objects.filter(description__contains = keyword) | Speaker.objects.filter(name__contains = keyword)
        return render(request, 'fullstack/speakers.html', {
            "v_speakers": results})